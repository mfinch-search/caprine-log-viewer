...
export default function LogEventTableRow({ logEvent }: { logEvent: Log }) {
    ...
    return (
        <tr>
            <td className="is-flex is-justify-content-space-between is-align-items-center is-size-6 " >
                <button aria-expanded={isExpanded ? 'true': 'false' } onClick={() => toggleExpansion()} className="button is-text">
                    <span className='mr-1'>{isExpanded ? UNICODE_DOWN_ARROW : UNICODE_RIGHT_ARROW}</span>
                </button>
                <span>{logEvent._time} </span>
            </td>
            {isExpanded ?
